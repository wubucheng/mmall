...
import cn.devshare.common.TokenCache;
import cn.devshare.dao.UserMapper;
import cn.devshare.pojo.User;
import cn.devshare.service.IUserService;
import cn.devshare.util.MD5Util;
...
public class UserServiceImpl implements IUserService {
    ...
    @Override
    public ServerResponse<User> login(String usename, String password) {
        int resultCount = userMapper.checkUsername(usename);
        if (resultCount == 0) {
            return ServerResponse.createByErrorMessage("用户名不存在");
        }
        String md5Password = MD5Util.MD5EncodeUtf8(password);
        User user = userMapper.selectLogin(usename, md5Password);
        if (user == null) {
    ...
    public ServerResponse<String> checkValid(String str,String type){
        ...
        return ServerResponse.createBySuccessMessage("校验成功");//检验成功的结果是用户不存在
    }
